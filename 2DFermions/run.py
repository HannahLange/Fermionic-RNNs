...
def cost_fct(samples, model, J, t, boundaries, sz_tot, N_tot, N, symmetry=None):
    if symmetry != None:
...
for epoch in range(1, n_epochs + 1):
    ...
    N  = o.get_particle_no(samples, model, device).mean(axis=0).detach()
    Elocs, cost, log_probs, phases = cost_fct(samples, model, J, t, bounds, sz_tot, N_tot, N, symmetry=sym)
    if max_grad != None:
