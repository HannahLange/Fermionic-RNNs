...
def compute_gradient_with_curvature(Ore, Oim, E, model, epsilon):
    ...
    T_ = T + torch.diag(torch.ones(T.size(0), device=T.device, dtype=torch.complex128)*epsilon)  
    Tinv = invert(T_, O, E) 
    ...
    inversion_error = torch.real(torch.linalg.norm((torch.mv(T.float(),TinvE.float())-E.float())))/(T.size(0)*T.size(1)) 
    tdvp_error = torch.real(torch.linalg.norm((torch.mv(Ore.float(),δ.float()))-E.real.float()))/E.size()[0] 
...
def run_sr(model, E, samples, optimizer, epsilon=1e-4, scheduler=None):
    ...
    if scheduler is not None:
        scheduler.step()
    return cost, inversion_error, tdvp_error, Ore, Oim
