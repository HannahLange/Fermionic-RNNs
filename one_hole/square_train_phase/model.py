...
class Model(nn.Module):
    ...
    def init_EDampls(self, Nh, obcX, obcY, t, Jp, Jz):
        d = {"open": 1, "periodic": 0}
        obcX = d[obcX]
        obcY = d[obcY]
        filename = "amplitudes_phases/bosonic_ampl_phases_"+str(self.N_x)+"x"+str(self.N_y)+"_Nh="+str(Nh)+"_boundsx="+str(obcX)+"_boundsy="+str(obcY)+"t="+str(int(t))+"_Jp="+str(int(Jp))+"_Jz="+str(int(Jz))+".csv"
        data = pd.read_csv(filename, header=None)
        self.EDampls = torch.tensor(data[0].values, device=self.device)
        configs = torch.tensor(data.values[:,2:18], device=self.device)
        self.EDconfigs = torch.reshape(configs, (self.EDampls.size(0), self.N_x, self.N_y))
    ...
    def sample(self, num_samples):
        ...
        """
        indices = torch.multinomial(torch.exp(self.EDampls), num_samples) 
        samples = self.EDconfigs[indices]
        samples[samples ==-1] = 2
        return samples
    ...
    def log_probabilities(self, samples):
        ...
        #IF WE LOAD THE PHASE!!!!!
        #print(log_probs_ampl)
        log_probs_ampl = h.get_ampl(samples, self.EDampls)
