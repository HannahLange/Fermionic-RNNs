...
def shift_samples(samples, s):
    shifted_x = samples.clone()
    shifted_x[:,s:,:] = samples[:,:-s,:]
    shifted_x[:,:s,:] = samples[:,-s:,:]

    shifted_y = samples.clone()
    shifted_y[:,:,s:] = samples[:,:,:-s]
    shifted_y[:,:,:s] = samples[:,:,-s:]

    return shifted_x, shifted_y

def calculate_momentum(samples, log_probs, log_phases, model, shift=1):
    ns_x, ns_y = shift_samples(samples, s=shift)
    ...
    Px = torch.log(expectation_x)*1j/shift
    Py = torch.log(expectation_y)*1j/shift
