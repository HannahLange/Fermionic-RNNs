...
if load_model:
    print("check if ../"+fol+"model_params"+fol_ext+"_no_antisym.pt exists.")
    if os.path.exists("../"+fol+"model_params"+fol_ext+"_no_antisym.pt"):
        print("load ../"+fol+"model_params"+fol_ext+"_no_antisym.pt")
        model.load_state_dict(torch.load("../"+fol+"model_params"+fol_ext+"_no_antisym.pt")) 
        fol_ext += "_pretrained"
...
while epoch <= num_epochs:
    ...
    if epoch < warmup_steps+1 or not minSR:
        lamb = 50*np.log10(1+9*epoch/1000)
        print(lamb)
        Elocs, cost, log_probs, phases, Px, Py  = momentum_cost_fct(samples, model, device, H, params, bounds_x, bounds_y, kx, ky, lamb, T)
        optimizer.zero_grad()
    ...
    if ((np.abs(np.real(Px))-kx)**2) <1e-4 or countdown!=0:
