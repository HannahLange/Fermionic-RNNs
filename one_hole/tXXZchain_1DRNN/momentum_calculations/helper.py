...
def momentum_cost_fct(samples, model, device, H, params, bounds_x, bounds_y, kx, ky, lamb, T):
    ...
    cost += 2 * torch.real((torch.conj(log_psi) * (e_loc_corr.detach().to(torch.complex128)))).mean(axis=0)
    if  ((torch.abs(torch.real(px))-kx)**2) <0.001:
        #lamb += 10
        cost += 0.5*lamb*((torch.abs(torch.imag(px))-0)**2) #+(torch.abs(torch.imag(py))-0)**2)
        print(0.5*lamb* ((torch.abs(torch.imag(px))-0)**2)) #+(torch.abs(torch.imag(py))-0)**2))
