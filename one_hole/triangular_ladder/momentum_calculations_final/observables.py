...
def shift_samples(samples):
    ...
    shifted_x[:,1:,:] = samples[:,:-1,:]
    shifted_x[:,:1,:] = samples[:,-1:,:]

    shifted_2x = samples.clone()
    shifted_2x[:,2:,:] = samples[:,:-2,:]
    shifted_2x[:,:2,:] = samples[:,-2:,:]

    return shifted_x, shifted_2x

def calculate_momentum(samples, log_probs, log_phases, model):
    ns_x, ns_2x = shift_samples(samples)
    ns = torch.cat([ns_x, ns_2x])

    ns_2D = torch.zeros((ns.size(0),2,9))
    for i in range(18):
        ns_2D[:,i%2,i//2] = ns[:,i,0]
    nprobs, nphases = model.log_probabilities(ns_2D)
    expect_x = torch.exp(0.5*(nprobs[:samples.size(0)]-log_probs)+1j*(nphases[:samples.size(0)]-log_phases))
    expect_2x = torch.exp(0.5*(nprobs[samples.size(0):]-log_probs)+1j*(nphases[samples.size(0):]-log_phases))

    expectation_x = (torch.mean(expect_x, axis=0))
    expectation_2x = (torch.mean(expect_2x, axis=0))

    #print("T", expectation_x, expectation_y)

    Px = torch.log(expectation_x)*1j
    P2x = torch.log(expectation_2x)*1j
    return Px, P2x, expect_x, expect_2x
