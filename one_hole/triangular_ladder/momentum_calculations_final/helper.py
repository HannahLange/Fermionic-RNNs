...
def momentum_cost_fct(samples, model, device, H, params, bounds_x, bounds_y, kx, ky, lamb):
    ...
    cost += lamb * (((torch.real(px))-kx)**2) #+(torch.abs(torch.real(p2x))-k2x)**2) #/((kx**2+ky**2+0.1)*10)
    print(lamb * (((torch.real(px))-kx)**2)) #+(torch.abs(torch.real(p2x))-k2x)**2)) #/((ky**2+0.1)*10))
    #if ((torch.abs(torch.real(px))-kx)**2+(torch.abs(torch.real(p2x))-k2x)**2) <0.001:
    #    cost += 0.1 * ((torch.abs(torch.imag(px))-0)**2+(torch.abs(torch.imag(p2x))-0)**2)
    #    print(0.1 * ((torch.abs(torch.imag(px))-0)**2+(torch.abs(torch.imag(p2x))-0)**2))
    ...
    return Eloc, cost, log_probs, phases, px.mean(), p2x.mean()
