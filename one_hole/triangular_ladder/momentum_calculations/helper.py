...
def momentum_cost_fct(samples, model, device, H, params, bounds_x, bounds_y, kx, ky, lamb):
    ...
    log_psi = (0.5*log_probs+1j*phases)
    eloc_sum = (Eloc).mean(axis=0)
    #samples_1D = samples.clone()
    #samples_1D = torch.stack([samples_1D[:,j%2,j//2] for j in range(18)],axis=-1).reshape((samples.size(0),18,1))
    #px, p2x, expect_x, expect_y = o.calculate_momentum(samples_1D,log_probs, phases, model)
    ...
    if (torch.abs(torch.real(px))-kx)**2+(torch.abs(torch.real(py))-ky)**2 >0.0005:
        cost += lamb * (((torch.real(py))-ky)**2) #+(torch.abs(torch.real(p2x))-k2x)**2) #/((kx**2+ky**2+0.1)*10)
        print(lamb * (((torch.real(py))-ky)**2)) #+(torch.abs(torch.real(p2x))-k2x)**2)) #/((ky**2+0.1)*10))
    #if True: #((torch.abs(torch.real(px))-kx)**2) <0.001:
    #    cost += 0.02*lamb * ((torch.abs(torch.imag(px))-0)**2+(torch.abs(torch.imag(py))-0)**2)
    #    print(0.02*lamb * ((torch.abs(torch.imag(px))-0)**2+(torch.abs(torch.imag(py))-0)**2))
...
def parse_input(parser, H):
    ...
    if args.bounds == 0:
        ...
        args.boundy = 0
    print(sz_tot)
    return {"U": U, "t": t, "Jp": Jp, "Jz": Jz}, density, Nx, Ny, b_dict[args.boundsx], b_dict[args.boundsy], {1: True, 0: False}[args.load_model], sz_tot, sym, antisym, hiddendim, kx, ky
