...
def momentum_cost_fct(samples, model, device, H, params, bounds_x, bounds_y, kx, ky, lamb):
    ...
    if ((torch.abs(torch.real(px))-kx)**2+(torch.abs(torch.real(py))-ky)**2) <0.001:
        cost += lamb* ((torch.abs(torch.imag(px))-0)**2+(torch.abs(torch.imag(py))-0)**2)
        print(lamb * ((torch.abs(torch.imag(px))-0)**2+(torch.abs(torch.imag(py))-0)**2))
