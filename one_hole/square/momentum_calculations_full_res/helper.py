...
def momentum_cost_fct(samples, model, device, H, params, bounds_x, bounds_y, kx, ky, lamb):
    ...
    print(lamb * ((torch.abs(torch.real(px))-kx)**2+(torch.abs(torch.real(py))-ky)**2)/((kx**2+ky**2+0.1)*10))
    if kx+ky>1 and ((torch.abs(torch.real(px))-kx)**2+(torch.abs(torch.real(py))-ky)**2) <0.01:
        cost += 0.05*lamb* ((torch.abs(torch.imag(px))-0)**2+(torch.abs(torch.imag(py))-0)**2)
        print(0.05*lamb* ((torch.abs(torch.imag(px))-0)**2+(torch.abs(torch.imag(py))-0)**2))
    #if ((torch.abs(torch.real(px))-kx)**2+(torch.abs(torch.real(py))-ky)**2) <0.01:
    #    cost += 10*lamb* ((torch.abs(torch.imag(px))-0)**2+(torch.abs(torch.imag(py))-0)**2)
    #    print(10*lamb * ((torch.abs(torch.imag(px))-0)**2+(torch.abs(torch.imag(py))-0)**2))
