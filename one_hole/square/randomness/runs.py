...
for seed in [1234, 1235, 1236,1237]:
    for density in [1,1-1/(Nx*Ny)]: #,1-4/(Nx*Ny),1-6/(Nx*Ny), 1-8/(Nx*Ny)]: #, 1-1/(Nx*Ny),1-2/(Nx*Ny), 1-3/(Nx*Ny)]:
        ...
        with open("mljob.sh", "r") as f:
            data = f.read()
        data = data.replace(".py", ".py -Nx "+str(Nx)+" -Ny "+str(Ny)+" -den "+str(density)+" -t "+str(t)+" -Jp "+str(Jp)+" -Jz "+str(Jz)+" -boundsx "+str(bounds_x)+" -boundsy "+str(bounds_y)+" -load "+str(load_data)+" -antisym "+str(antisym)+" -hd "+str(hiddendim)+" -sym 0 -seed "+str(seed))
