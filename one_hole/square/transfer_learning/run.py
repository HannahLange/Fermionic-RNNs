...
fol_ = "../"+str(Nx)+"x"+str(Ny)+"_qubits/"+bounds+"/Jp="+str(float(params["Jp"]))+"Jz="+str(float(params["Jz"]))+"t="+str(float(0.0))+"den="+"{:.2f}".format(1.0)+"/"
if load_model:
    print("check if "+fol_+"model_params"+fol_ext+".pt exists.")
    if os.path.exists(fol_+"model_params"+fol_ext+".pt"):
        print("load "+fol_+"model_params"+fol_ext+".pt")
        model.load_state_dict(torch.load(fol_+"model_params"+fol_ext+".pt")) 
