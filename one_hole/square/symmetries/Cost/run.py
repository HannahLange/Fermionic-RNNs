...
if load_model:
    ...
    if os.path.exists(fol+"model_params"+fol_ext+".pt"):
        ...
        fol_ext += "_"+str(n) #fol_ext.split("_5")[0]+"_6"
    if os.path.exists(fol+"model_params"+fol_ext+"_5.pt"):
        print("load "+fol+"model_params"+fol_ext+"_5.pt")
        model.load_state_dict(torch.load(fol+"model_params"+fol_ext+"_5.pt")) 
        fol_ext += "_6"
...
while epoch <= num_epochs:
    ...
    if epoch < warmup_steps+1 or not minSR:
        ...
        Elocs = []
        for batch in range(int(n_samples/batchsize)):
            Eloc, c, log_probs, phases, p_diffs  = cost_fct(samples[batch*batchsize:(batch+1)*batchsize], model, device, H, params, bounds_x, bounds_y,  mu_sym_log, sym, T, antisym)
            cost += c
            Elocs.append(Eloc)
    ...
    model.zero_grad()
    if p_diffs != None:
        p_diffs_training.append(p_diffs.detach().cpu().numpy())
