...
def get_Eloc(model, parameters, samples, RNN, boundaries_x, boundaries_y, symmetry, antisym):
    ...
    symmetric_samples_new = get_symmetric_samples(new_samples, symmetry, device)
    queue_samples = torch.zeros(((length+1), numsamples, Nx, Ny), dtype=torch.int32, device=device)  
    queue_samples_2 = torch.zeros(((length+1), numsamples, Nx, Ny), dtype=torch.int32, device=device) 
    queue_samples_3 = torch.zeros(((length+1), numsamples, Nx, Ny), dtype=torch.int32, device=device) 
    queue_samples_4 = torch.zeros(((length+1), numsamples, Nx, Ny), dtype=torch.int32, device=device) 
    #Eloc2 = diag_me.to(torch.complex64)
    queue_samples[0] = samples
    queue_samples[1:length+1] = new_samples
    queue_samples_2[0] = symmetric_samples[:samples.size(0)]
    queue_samples_2[1:length+1] = symmetric_samples_new[:,:new_samples.size(1)]
    queue_samples_3[0] = symmetric_samples[samples.size(0):2*samples.size(0)]
    queue_samples_3[1:length+1] = symmetric_samples_new[:,new_samples.size(1):2*new_samples.size(1)]
    queue_samples_4[0] = symmetric_samples[2*samples.size(0):3*samples.size(0)]
    queue_samples_4[1:length+1] = symmetric_samples_new[:,2*new_samples.size(1):3*new_samples.size(1)]

    queue_samples_reshaped = torch.reshape(queue_samples, [queue_samples.size()[0]*numsamples, Nx, Ny])
    queue_samples_reshaped_2 = torch.reshape(queue_samples_2, [queue_samples.size()[0]*numsamples, Nx, Ny])
    queue_samples_reshaped_3 = torch.reshape(queue_samples_3, [queue_samples.size()[0]*numsamples, Nx, Ny])
    queue_samples_reshaped_4 = torch.reshape(queue_samples_4, [queue_samples.size()[0]*numsamples, Nx, Ny])

    log_probs, phases = RNN.log_probabilities(queue_samples_reshaped.to(torch.int64))
    log_probs_2, phases2 = RNN.log_probabilities(queue_samples_reshaped_2.to(torch.int64))
    log_probs_3, phases3 = RNN.log_probabilities(queue_samples_reshaped_3.to(torch.int64))
    log_probs_4, phases4  = RNN.log_probabilities(queue_samples_reshaped_4.to(torch.int64))
    psi = torch.sqrt((torch.exp(log_probs)+torch.exp(log_probs_2)+torch.exp(log_probs_3)+torch.exp(log_probs_4))/4).to(torch.complex128)
    averaged_phases = (torch.exp(1j*phases)+torch.exp(1j*phases2)+torch.exp(1j*phases3)+torch.exp(1j*phases4))/4
    psi *= torch.exp(1j * torch.angle(averaged_phases))
    log_probs = torch.log(psi*psi.conj()).to(torch.float64)
    phases = torch.log(psi).imag.to(torch.float64)
...
def get_symmetric_samples(original_samples, symmetry, device):
    ...
    if len(samples.size())==3:
        symmetric_samples = torch.zeros((samples.size()[0]*3,samples.size()[1], samples.size()[2]), dtype = torch.int32).to(device)  
        l = int(samples.size()[0])
        symmetric_samples[:l]    = torch.rot90(samples,    1, [1,2])
        symmetric_samples[l:2*l] = torch.rot90(samples, 2, [1,2])
        symmetric_samples[2*l:]  = torch.rot90(samples,  3, [1,2])
    else:
        ...
        symmetric_samples[:,:l]    = torch.rot90(samples,    1, [2,3])
        symmetric_samples[:,l:2*l] = torch.rot90(samples, 2, [2,3])
        symmetric_samples[:,2*l:]  = torch.rot90(samples,  3, [2,3])
