...
def get_Eloc(model, parameters, samples, RNN, boundaries_x, boundaries_y, symmetry, antisym):
    ...
    log_probs_2, _ = RNN.log_probabilities(queue_samples_reshaped_2.to(torch.int64))
    log_probs_3, _ = RNN.log_probabilities(queue_samples_reshaped_3.to(torch.int64))
    log_probs_4, _ = RNN.log_probabilities(queue_samples_reshaped_4.to(torch.int64))
    log_probs = torch.log((torch.exp(log_probs)+torch.exp(log_probs_2)+torch.exp(log_probs_3)+torch.exp(log_probs_4))/4)
