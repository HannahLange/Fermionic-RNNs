                    ...
                    if antisym:
                        if j % 2 == 0: #go from left to right    
                            for i2 in range(i+1,Nx):
                                ...
                                p1[samples[:,i2,j]==2] = 1
                                p1[samples[:,i2,j]==0] = 0
                                p2[samples[:,i2,(j+1)%Ny]==-1] = 1
                                p2[samples[:,i2,(j+1)%Ny]==2] = 1
                                p2[samples[:,i2,(j+1)%Ny]==0] = 0
                                P += (p1+p2)
...
def get_Eloc(model, parameters, samples, RNN, boundaries_x, boundaries_y, symmetry, antisym):
    ...
    if t!= 0 and model == "tJ":
        phases_xprime = phases_reshaped[int(length/2+1):length+1,:].clone()
        phases_reshaped[int(length/2+1):length+1,:] = signs + phases_reshaped[int(length/2+1):length+1,:]
    RNN_signs = torch.zeros((length+1-int(length/2+1), numsamples)).to(torch.complex64).to(device)
    for i in range(1,(length+1)):
        if i>=int(length/2+1) and i<length+1 and t!= 0 and model == "tJ": 
            RNN_signs[i-int(length/2+1),:] = (torch.exp(1j*(phases_xprime[i-int(length/2+1),:]-phases_reshaped[0,:])))
    ...
    if symmetry != None:
        return Eloc, log_probs_reshaped[0], phases_reshaped[0], symmetric_samples, log_probs_reshaped[length+1], phases_reshaped[length+1], torch.exp(1j*signs), RNN_signs
    else:
        return Eloc, log_probs_reshaped[0], phases_reshaped[0], torch.exp(1j*signs), RNN_signs
