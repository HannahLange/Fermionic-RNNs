...
def cost_fct(samples, model, device, H, params, bounds_x, bounds_y, N, mu, mu_sz, mu_sym, sz_tot, su2_sym, N_tot, symmetry, T, antisym):
    ...
    if T != None and T!= 0:
        cost += 4*T*(torch.mean(torch.real(log_probs)*torch.real(log_probs.detach()))-torch.mean(torch.real(log_probs))*torch.mean(torch.real(log_probs.detach())) )
    if symmetry != None and mu_sym != 0:
        p_diff = (torch.exp(log_psi)**2-torch.exp(sym_log_psi)**2)/(0.5*(torch.exp(log_psi)**2+torch.exp(sym_log_psi)**2))
        #cost += mu_sz * 400 * torch.real((p_diff).detach() * (torch.real((torch.conj(log_psi)-(torch.conj(sym_log_psi)))))).mean(axis=0)
        e_loc_corr += (mu_sym *(p_diff)**2)
