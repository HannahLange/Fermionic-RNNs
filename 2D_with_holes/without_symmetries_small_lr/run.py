...
if load_model:
    if os.path.exists(fol+"model_params.pt"):
        model.load_state_dict(torch.load(fol.split("Jp=")[0]+"Jp=0.0Jz=0.0t="+fol.split("t=")[1]+"model_params.pt"))    
...
while epoch <= num_epochs:
    ...
    if epoch > warmup_steps+annealing_steps/2:
        optimizer.param_groups[0]['lr'] = lr/np.sqrt(1+(epoch-(warmup_steps+annealing_steps/2))/2000)
        sz_tot = sz_total
        mu_sz_log = np.log10(1+9*(epoch-(warmup_steps+annealing_steps/2))/2000)*mu_sz
    if epoch > warmup_steps+annealing_steps:
        sym = sym_
        mu_sym_log = np.log10(1+9*(epoch-(warmup_steps+annealing_steps))/2000)*mu_sym
    ...
    if load_model: 
        ...
        params["Jp"] = min(np.log10(1+9*epoch/5000), 1)
        params["Jz"] = min(np.log10(1+9*epoch/5000), 1)
        print(params["Jz"])
