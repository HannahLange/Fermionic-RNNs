...
def cost_fct(signtrain, samples, model, device, H, params, bounds_x, bounds_y, N, mu, mu_sz, mu_sym, sz_tot, su2_sym, N_tot, symmetry, T, antisym, mse_loss):
    cost = 0
    if symmetry != None:
        Eloc, log_probs, phases, sym_samples, sym_log_probs, sym_phases, signs, RNN_signs = get_Eloc(H,params,samples, model, bounds_x, bounds_y, symmetry, antisym)
        sym_log_psi = (0.5*sym_log_probs+1j*sym_phases)
    else:
        Eloc, log_probs, phases, signs, RNN_signs = get_Eloc(H,params,samples, model, bounds_x, bounds_y, symmetry, antisym)
    log_psi = (0.5*log_probs+1j*phases)
    if signtrain:
        signs = torch.reshape(signs, (signs.size()[0]*signs.size()[1],))
        RNN_signs = torch.reshape(RNN_signs, (RNN_signs.size()[0]*RNN_signs.size()[1],))
        cost = 10* mse_loss(torch.real(signs), torch.real(RNN_signs))
        p_diff = None
