...
def cost_fct(samples, model, device, H, params, bounds_x, bounds_y, mu_sym, symmetry, T, antisym, true_samples=None, kl_loss=None):
    cost = 0
    if symmetry != None:
        ...
        print((Eloc).mean(axis=0))
    else:
        Eloc, log_probs, phases = get_Eloc(H,params,samples, model, bounds_x, bounds_y, symmetry, antisym)
    log_psi = (0.5*log_probs+1j*phases)
    if kl_loss != None:
        true_freq = Counter(true_samples)
        p_true = []
        configs = []
        for config in true_freq.keys():
            p_true.append(true_freq[config]/len(true_samples))
            config = torch.tensor([float(it) for it in config.split(",")])
            config = torch.reshape(config, (samples.size(1), samples.size(2)))
            configs.append(config)
        configs = torch.stack(configs, axis=0).long().to(device)
        configs[configs==-1] = 2
        p_true = torch.tensor(p_true, device=device)
        p_RNN, _ = model.log_probabilities(configs)
        print(len(p_true), p_true[0:3],torch.exp(p_RNN[0:3])) 
        cost = kl_loss(p_RNN, p_true)
        p_diff = None
    else:
