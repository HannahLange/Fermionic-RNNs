...
class Model(nn.Module):
    def __init__(self, input_size, system_size_x, system_size_y, hidden_dim, weight_sharing, device):
    ...
    def init_hidden(self, batch_size):
        ...
        hidden = torch.empty((batch_size, self.hidden_dim), dtype=torch.float32).to(self.device).normal_(mean=0, std=0.01)
        return hidden
        ...
        for ny in range(self.N_y):
            if ny % 2 == 0: #go from left to right    
                for nx in range(self.N_x):
                    direction = [-1,-1]
                    samples[nx][ny], sigma, hidden = self._gen_samples(nx, ny, direction, inputs, hidden_inputs, samples, num_samples, num)
                    inputs[str(nx)+str(ny)] = sigma
    ...
    def log_probabilities(self, samples):
        ...
        for ny in range(self.N_y):
            if ny % 2 == 0: #go from left to right
                for nx in range(self.N_x):
                    direction = [-1,-1]
                    sigma, ampl_probs[nx][ny], phase_probs[nx][ny], hidden = self._gen_probs(nx, ny, direction, samples, inputs, hidden_inputs, num)
                    ohs[nx][ny] = sigma
                    inputs[str(nx)+str(ny)] = sigma
                    hidden_inputs[0][str(nx)+str(ny)] = hidden[0]
                    if self.n_layer > 1: hidden_inputs[1][str(nx)+str(ny)]  = hidden[1]
